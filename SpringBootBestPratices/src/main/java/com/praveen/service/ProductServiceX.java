...
import com.praveen.exception.ProductNotFoundException;
import com.praveen.exception.ProductServiceBusinessException;
import com.praveen.repository.ProductRepository;
import com.praveen.util.ValueMapper;
...
public class ProductServiceX {
    ...
    public ProductResponseDTO createNewProduct(ProductRequestDTO productRequestDTO) throws ProductServiceBusinessException {
        ...
        try {
            ...
            ProductX product = ValueMapper.convertToEntity(productRequestDTO);
            log.debug("ProductService:createNewProduct request parameters {}", ValueMapper.jsonAsString(productRequestDTO));

            ProductX productResults = productRepository.save(product);
    ...
    public List<ProductResponseDTO> getProducts() throws ProductServiceBusinessException {
        ...
        try {
            ...
            List<ProductX> productList = productRepository.findAll();
...
//            }
            ...
            ProductX product = productRepository.findById(productId)
    ...
    public Map<String, List<ProductResponseDTO>> getProductsByTypesBeforeJava8() {
        ...
        List<ProductX> productList = productRepository.findAll(); //2 nd

        for (String type : productTypes) {
            List<ProductResponseDTO> productResponseDTOList = new ArrayList<>();
            for (ProductX product : productList) {
